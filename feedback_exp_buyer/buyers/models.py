        ...
        for group_id in available_groups:
            ...
            if assign_new_item and available_items:
                ...
                valid_seen_items = [
                    ...
                    # Must be first seen by another group
                ]
                ...
                if valid_seen_items:
                    ...
                    print(f"[ERROR] No valid seen item available for Group {group_id} in round {self.round_number}")
                    raise Exception(f"Critical Error: No valid seen item for Group {group_id}")

            if not assigned_item:
                print(f"[ERROR] No item assigned to Group {group_id}. This is a critical issue.")
                raise Exception(f"Critical Error: No valid item for Group {group_id} in round {self.round_number}")
